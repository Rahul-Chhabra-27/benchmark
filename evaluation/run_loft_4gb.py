"""Run one worker's LOFT no-press baseline and 4 GB KV-cache budget."""
...
def main() -> None:
    ...
    if not isinstance(config.data_dir, list):
        raise ValueError("LOFT 4 GB config requires data_dir to be a task list")

    assigned_tasks = config.data_dir[args.worker_id :: args.worker_count]
    print(
        f"4 GB worker {args.worker_id}/{args.worker_count} assigned tasks: "
